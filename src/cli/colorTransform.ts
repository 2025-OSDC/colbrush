...
export function generateCandidates(hex: string) {
    ...
    for (let i = 1; i <= CANDIDATE_COUNT / 2; i++) {
        candidates[i * 2 - 1] = chroma(hex)
            .set('hcl.h', (baseHue + (HUE_STEP / CANDIDATE_COUNT) * i) % 360)
            .hex();
        candidates[i * 2] = chroma(hex)
            .set(
                'hcl.h',
...
export async function requestColorTransformation(
    ...
    const colorKeys = Object.keys(filteredVariables);
    const baseColorsArray = Object.values(filteredVariables).map((v) => v.base);
    // 섹션: 후보 색상 생성 (각 변수에 대한 후보군 준비)
    progress?.startSection('후보 색상 생성');
    const candidateList: string[][] = [];
    for (let i = 0; i < baseColorsArray.length; i++) {
        candidateList[i] = generateCandidates(baseColorsArray[i]);
        progress?.update(((i + 1) / baseColorsArray.length) * 100);
    ...
    // 섹션: 시뮬레이션/최적화 (각 시각 유형별 0→100)
    progress?.startSection('Optimizing — deuteranopia');
    const colors_d = findOptimalColorCombination(
        colorKeys,
        baseColorsArray,
        candidateList,
        'deuteranopia'
    ...
    progress?.startSection('Optimizing — protanopia');
    const colors_p = findOptimalColorCombination(
        colorKeys,
        baseColorsArray,
        candidateList,
        'protanopia'
    ...
    progress?.startSection('Optimizing — tritanopia');
    const colors_t = findOptimalColorCombination(
        colorKeys,
        baseColorsArray,
        candidateList,
        'tritanopia'
    ...
    const themes = visions.map((vision) => ({
        vision,
        variables: Object.entries(colorsMap[vision]).reduce(
            (acc, [varName, baseHex]) => {
                acc[varName] = inferRich(varName, baseHex);
                return acc;
            },
...
function isValidColors(colors: string[], blind: (hex: string) => string) {
    for (let i = 0; i < colors.length; i++) {
        for (let j = i + 1; j < colors.length; j++) {
            if (chroma.deltaE(blind(colors[i]), blind(colors[j])) < THRESHOLD)
                return false;
        }
    }
